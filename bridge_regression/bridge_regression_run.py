...
from bridge_regression import train_bridge_model, evaluate_cross_validation
...
def analyze_auto_mpg():
    ...
    model = train_bridge_model(train_x, test_x, train_y, test_y, alpha=0.1, q=0.5)
...
def analyze_energy_efficiency():
    ...
    model = train_bridge_model(train_x, test_x, train_y, test_y, alpha=0.1, q=0.5)
    cv_results = evaluate_cross_validation(model, features_cooling, target_cooling)
...
def analyze_seoul_bike():
    ...
    print("Target (Rented Bike Count) mean:", target.mean())
    print("Target (Rented Bike Count) std:", target.std())
    train_x, test_x, train_y, test_y = train_test_split(features, target, test_size=0.2, random_state=42)
    model = train_bridge_model(train_x, test_x, train_y, test_y, alpha=0.1, q=0.5)
    cv_results = evaluate_cross_validation(model, features, target)
    print("Cross-Validation Results:", cv_results)
...
def analyze_boston_housing():
    """
    Runs Bridge regression on the Boston Housing dataset and evaluates performance.
    """
...
def execute_all_datasets():
    """
    Executes Bridge regression analysis for all datasets and prints results.
    """
    print("\n=== Auto MPG Bridge Regression Analysis ===")
    analyze_auto_mpg()
    print("\n=== Energy Efficieny Bridge Regression Analysis ===")
    analyze_energy_efficiency()
    print("\n=== Seoul Bike Bridge Regression Analysis ===")
    analyze_seoul_bike()
    print("\n=== Boston Housing Bridge Regression Analysis ===")
