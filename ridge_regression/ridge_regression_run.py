...
from ridge_regression import train_ridge_model, evaluate_cross_validation


def extract_auto_mpg_features():
    data = auto_mpg.prepare_mpg_data()
    features = data.drop(columns="brand").drop(columns="mpg")
    target = data["mpg"]
...
def analyze_auto_mpg():
    ...
    model = train_ridge_model(train_x, test_x, train_y, test_y)
    results = evaluate_cross_validation(model, features, target)
...
def execute_all_datasets():
    """
    Executes Ridge regression analysis for all datasets and prints results.
    """
    print("\n=== Auto MPG Ridge Regression Analysis ===")
    analyze_auto_mpg()
    print("\n=== Energy Efficiency Ridge Regression Analysis ===")
    analyze_energy_efficiency()
    print("\n=== Seoul Bike Ridge Regression Analysis ===")
    analyze_seoul_bike()
